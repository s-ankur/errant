...
class Alignment:
    ...
    def __init__(self, orig, cor, lev=False):
        ...
        self.temp1=[]
        self.temp2=[]
        for sent in self.orig.sentences:
          for word in sent.words:
            self.temp1.append(word)
        for sent in self.cor.sentences:
          for word in sent.words:
            self.temp2.append(word)
    ...
    def align(self, lev):
        ...
        o_len = len(self.temp1)
        c_len = len(self.temp2)
        # Lower case token IDs (for transpositions)
        o_low = [o for o in self.temp1]
        c_low = [c for c in self.temp2]
        o_low1= [o.text for o in self.temp1]
        c_low1= [c.text for c in self.temp2]
        ...
        for i in range(o_len):
            for j in range(c_len):
                # Matches
                if self.temp1[i].text == self.temp2[j].text:
                ...
                # Non-matches
                    ...
                    # Linguistic Damerau-Levenshtein
                        ...
                        while i-k >= 0 and j-k >= 0 and \
                            ...
                            if sorted(o_low1[i-k:i+1]) == sorted(c_low1[j-k:j+1]):
    ...
    def get_sub_cost(self, o, c):
        ...
        if o.text == c.text: return 0
        # Lemma cost
        if o.lemma == c.lemma: lemma_cost = 0
        else: lemma_cost = 0.499
        # POS cost
        if o.upos == c.upos: pos_cost = 0
        elif o.upos in self._open_pos and c.upos in self._open_pos: pos_cost = 0.25
        else: pos_cost = 0.5
        # Char cost
    ...
    def __str__(self):
        orig = " ".join(["Orig:"]+[tok.text for tok in self.temp1])
        cor = " ".join(["Cor:"]+[tok.text for tok in self.temp2])
